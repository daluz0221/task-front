...
export const useCategoryStore = create<CategoryState>((set) => ({
    ...
    addCategory: async (name) => {
        try {
            const res = await fetchWithAuth('https://youragenda.app/api/categories/', {
                method: "POST", 
                body: JSON.stringify({
                    name
                })
    ...
    fetchCategoryById: async(id: string)=>{
        try {
            const res = await fetchWithAuth(`https://youragenda.app/api/categories/category/${ id }`);
    ...
    deleteCategory: async (id: string) => {
        try {
            const res = await fetchWithAuth(`https://youragenda.app/api/categories/delete/${id}`, {
                method: "PUT"
            })
            const data = await res.json()
    ...
    updateCategory: async({id, name}: {id: string, name: string}) => {
        ...
        try {
            const res = await fetchWithAuth(`https://youragenda.app/api/categories/update/${id}`, {
                method: "PUT",
